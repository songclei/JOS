...
trap_dispatch(struct Trapframe *tf)
{
	...
	// LAB 3: Your code here.
	#ifdef CHALLENGE
	if((tf->tf_cs & 3) == 3 && curenv->env_exception_upcall != 0
		&& curenv->set_handler[tf->tf_trapno] == true) {
		// handle with user exception handler
		user_defined_handler(tf);
	}
...
#ifdef CHALLENGE
void 
user_defined_handler(struct Trapframe *tf) 
{
	uint32_t fault_va;
