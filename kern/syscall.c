...
sys_env_set_exception_upcall(envid_t envid, void *func)
{
	...
	given_env->env_exception_upcall = func;

	return 0;
}

static int
sys_env_set_exception_handler(envid_t envid, int32_t exception_num)
{
	struct Env *given_env;
	...
	given_env->set_handler[exception_num] = true;
...
#ifdef CHALLENGE5
static int
sys_env_set_pte(envid_t envid, void *addr, pte_t contents)
{
	// only file system environment can call this system call
	...
	if(envid2env(envid, &given_env, 0) < 0)
		return -E_BAD_ENV;

	// find the page table entry for virtual address addr
	pte = pgdir_walk(given_env->env_pgdir, addr, true);
	if(pte == NULL)
		return -E_NO_MEM;

	// change the page table entry's content
	*pte = contents;
