...
serve_load(envid_t envid, union Fsipc *ipc)
{
	...
	struct Fsret_load *ret = &ipc->loadRet;
	int r;

	// if it is not a shared page, then we should 
	// read from the disk and then copy to the ret buffer
	if(req->req_shared == false) {
		memmove(ret->ret_buf, diskaddr(req->req_blockno), PGSIZE);
	}
	// if it is a shared page, we should map the physical page
	// dirct to addr
	else {
		if((r = sys_page_map(0, diskaddr(req->req_blockno), envid,
			(void *)((int)req->req_addr&(~0xFFF)), req->req_perm|PTE_P)) < 0) 
			return r;
	}
...
serve_mmap(envid_t envid, struct Fsreq_mmap *req)
{
	...
	int r;
	pte_t contents;
	void *addr = req->req_start;
	size_t length = req->req_length;
	int offset = req->req_offset;
	int perm = req->req_perm;
	...
	for(int filebno = offset/BLKSIZE; filebno < (offset+length)/BLKSIZE; ++filebno) {
		char *blk;

		if((r = file_get_block(f, filebno, &blk)) < 0)
			return r;
		...
		filebno = ((int)blk - DISKMAP) / BLKSIZE;

		contents = (filebno << PGSHIFT) + perm;

		if((r = sys_env_set_pte(envid, addr, contents)) < 0)
			return r;
